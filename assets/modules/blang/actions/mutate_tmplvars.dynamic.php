...
$id = isset($_REQUEST['elemId']) ? intval($_REQUEST['elemId']) : 0;
...
function deletedocument() {
    ...
    if(confirm("<?php echo $_lang['confirm_delete_tmplvars']; ?>")==true) {
        documentDirty=false;
        document.location.href="<?=$GLOBALS['moduleUrl'] ?>action=paramDelete&elemId="+document.mutate.elemId.value;
    }
}
...
<form name="mutate" method="post" action="index.php?a=112&id=<?= $_GET['id'] ?>&action=paramSave" enctype="multipart/form-data">
<?php
...
<input type="hidden" name="elemId" value="<?php echo $content['id'];?>">
...
<input type="hidden" name="params" value="<?php echo $modx->htmlspecialchars($content['display_params']);?>">
    ...
    <div id="actions">
          ...
          <?php } ?>
              <li id="Button5" class="transition"><a href="#" onclick="documentDirty=false;document.location.href='<?= $GLOBALS['moduleUrl'] ?>';"><img src="<?php echo $_style["icons_cancel"] ?>" /> <?php echo $_lang['cancel']?></a></li>
          </ul>
    </div>
...
<table>
  ...
  <tr>
    ...
    <tr>
        <th><?php echo $_lang['tmplvars_tab']; ?></th>
        <td><input name="tab" type="text" maxlength="255" value="<?php echo $modx->htmlspecialchars($content['tab']);?>" class="inputBox" style="width:300px;" onChange="documentDirty=true;"></td>
    </tr>
  <tr>
...
<?php
        $rs = $modx->db->select(
            ...
            sprintf("%s as tpl
                    ...
                    $modx->getFullTableName('site_templates'), $modx->getFullTableName('blang_tmplvar_templates'), $id, $modx->getFullTableName('categories')),
            '',
